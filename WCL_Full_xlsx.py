...
to pull the all dps rankings for any raid on any difficulty.  These are combined into an xlsx file. """
...
def full(boss, excel):
  ...
  df.to_excel(excel, boss['name'], index=False)

# main function that pulls for each boss and saves the xlsx file
def puller():
  writer = pd.ExcelWriter(raid + '_full.xlsx')
  for tier in zone:
    if tier['name'] == raid:
